...
如果我们不关心参数的类型或值，只关心是否存在任何参数，我们使用`any()`：
...
静态方法不会在实际对象上调用，因此普通的模拟对象无法模拟它们。Mockery支持类别名模拟，这些模拟代表了一个类名，在系统测试中通常会加载（通过自动加载或`require`语句）。这些别名会阻止加载（除非通过`require`语句 - 因此请使用自动加载！），并允许Mockery拦截静态方法调用并为其添加期望。
