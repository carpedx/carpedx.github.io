...
NoSQL，全称Non-relational SQL，泛指非关系型的数据库。它代表了一系列的、不同类型的相互关联的数据存储与处理的技术的集合
...
1. 数据大小：Memcached单个key-value的大小有限，一个value最大只支持1MB，key最大250个字符，而Redis最大支持512MB。
2. 数据类型：Memcache只支持简单的数据类型，如字符串、数字和图片等，而Redis支持复杂的数据类型，如哈希、列表、集合和有序集合等。
...
1. **String**：这是Redis最基础的数据结构，可以用来存储简单的字符串或json格式的字符串。同时，它也被用于分布式锁的实现，计数器、Session共享以及分布式ID的制作。
2. **Hash**：哈希表主要用于存储键值对，其更适合于存储对象。
3. **List**：Redis的列表可以作为栈和队列使用，可以用来存储类似微信公众号，微博等消息流数据。
4. **Set**：与列表类似，集合也可以存储多个元素，但集合中的元素不能重复。集合可以进行交集、并集和差集操作。
5. **Zset**：有序集合和集合类似，但有序集合中的元素是有序的。可以用来实现排行榜功能。
...
缓存雪崩是指缓存由于某些原因（例如宕机、cache服务挂掉或不响应）整体crash掉，导致大量请求到达后端数据库，从而导致数据库崩溃，整个系统崩溃，发生灾难。

缓存雪崩可能会发生的原因很多，例如缓存并发、缓存穿透和缓存颠簸等问题。这些问题的存在可能会导致缓存中的数据被清除掉或者无法访问，进而导致请求无法被正确处理，整个系统崩溃。

解决缓存雪崩问题的方法包括以下几种：

1. 设置不同的过期时间：通过错开缓存过期时间，避免缓存集中失效导致请求大量到达后端数据库。
2. 使用双机房部署：将缓存集群部署在两个不同的机房，部分机器在一个机房，另一部分机器在另一个机房，以避免单个机房的缓存服务崩溃导致整个系统崩溃。
3. 使用两套Redis Cluster：将两套Redis Cluster之间做一个数据同步，以避免一个集群崩溃导致整个缓存服务失效。
4. 使用ehcache本地缓存：当源服务访问出现故障时，可以利用ehcache本地缓存支撑一阵，避免缓存服务彻底崩溃。
...
1. MD5加密：
   MD5是一种散列函数，它将任意长度的数据转换成一个固定长度的哈希值。在PHP中，可以使用`md5()`函数对数据进行MD5加密。这种加密方式是单向的，无法解密，因此只能用于验证数据的完整性，而不能用于加密和解密数据。
2. SHA1加密：
   SHA1也是一种散列函数，与MD5类似，它将数据转换为一个固定长度的哈希值。SHA1比MD5更安全，但计算开销也更大。在PHP中，可以使用`sha1()`函数对数据进行SHA1加密。同样，SHA1是单向的，无法解密。
3. 对称加密：
   对称加密使用相同的密钥进行加密和解密。在PHP中，常用的对称加密算法有AES（Advanced Encryption Standard）、DES、3DES（Triple DES）等。这些算法都是块密码，将明文分成固定大小的块，然后对每个块进行加密。可以使用`openssl`相关函数对数据进行对称加密。
4. 非对称加密：
   非对称加密使用公钥和私钥来进行加密和解密。在PHP中，常用的非对称加密算法是RSA（Rivest-Shamir-Adleman）。RSA算法是公钥密码体系，它将明文分成块，使用公钥对每个块进行加密，然后接收方使用私钥进行解密。可以使用`openssl`相关函数对数据进行非对称加密。
...
3. 模块化设计：Nginx是高度模块化的设计，这意味着添加新的功能或模块相对简单。相比之下，Apache的模块较多，但Rewrite模块比Nginx的Rewrite模块功能弱。
4. 与FastCGI的兼容性：Nginx友好支持FastCGI，这对于处理动态内容和高性能的Web应用（如PHP）非常有用。而Apache对FastCGI的支持则相对较差。

总之，Nginx和Apache各有优点和适用场景。一般来说，对于需要处理高并发请求、动态内容和高性能Web应用的场景，建议使用Nginx。如果需要处理大型和小型并发量、且有大量静态内容要处理的场景，Apache也是一个不错的选择。
